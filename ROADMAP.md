...
- Seed data + demo accounts exercising the full chain end-to-end.

See `README.md` for demo accounts and `ARCHITECTURE.md`/`DATABASE.md` for
how it's built.

## Immediate next slice (P4 — Execution)

The natural continuation of the workflow already modeled:

1. **Purchase orders** — generated from an `ACCEPTED` quotation (the schema
   already has this as the natural trigger point).
2. **Contracts** — generic contract model with parties, milestones, documents.
3. **Deliveries** — track against a purchase order, with verification status.
4. **Milestones** — project milestones with planned/actual dates and payment
   linkage.
5. **Invoices** — from POs/contracts, with approval workflow.
6. **Payment records** — explicitly labeled as *transaction records*, not a
